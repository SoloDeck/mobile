...
void main() {
  group('MÀN 04 — cấu trúc theo figcaption', () {
    ...
    testWidgets('giá trị ước tính hiện ngay trên mỗi thẻ deal', (tester) async {
      ...
      expect(find.text('~9.000.000 ₫'), findsOneWidget);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(find.text('Vào 0 ngày trước'), findsOneWidget);
        expect(find.text('Vào 2 ngày trước'), findsOneWidget);
        expect(find.text('Vào 5 ngày trước'), findsOneWidget);
        expect(find.text('Vào 11 ngày trước'), findsOneWidget);
        expect(find.text('Vào 3 ngày trước'), findsOneWidget);
      },
    );
  });
  ...
  group('MÀN 04 — quy ước màu', () {
    ...
    testWidgets(
      ...
      (tester) async {
        await _pump(tester);
        // Xuất hiện hai lần: deal thứ nhất và thứ năm cùng rơi vào chu kỳ mock
        // "chưa chấm" (`index % 4 == 0`) — chỉ cần kiểm một cái đại diện.
        final chip = tester
  ...
  group('MÀN 04 — chuỗi hiển thị chép từ bản phác thảo', () {
    ...
    testWidgets('tổng ước tính định dạng theo cách người Việt viết', (
      ...
      await _pump(tester);
      // 15tr + 28tr + 9tr + 2tr + 5tr = 59tr — tổng cả 5 deal trả về từ
      // dealListProvider, không lọc theo giai đoạn.
      expect(find.text('59.000.000 ₫'), findsOneWidget);
      // `SectionLabel` tự in hoa (`text-transform: uppercase`), nên chuỗi
      // hiển thị viết hoa dù mã nguồn truyền vào viết thường.
      expect(find.text('5 LEAD · TỔNG ƯỚC TÍNH'), findsOneWidget);
    });
  });
  ...
  group('MÀN 04 — dữ liệu nối provider', () {
    ...
    testWidgets('trạng thái rỗng khi dealListProvider trả về danh sách rỗng', (
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [dealListProvider.overrideWith((ref) async => const [])],
